...
int main() {
    const std::string api_key = "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";
    ...
    conversation_history.push_back({
        {"role", "system"},
        {"content", "You are a helpful financial advisor. Provide sound financial advice and tips."}
    });
    ...
    while (true) {
        ...
        // Let's try writing to a temp file first as it's much more robust on Windows.
        ...
        std::string command = "curl -s -X POST https://openrouter.ai/api/v1/chat/completions "
        ...
        try {
            ...
            try {
                ...
                if (response_json.contains("error")) {
                     std::cerr << "API Error: " << response_json["error"].dump() << std::endl;
                } else if (response_json.contains("choices") && !response_json["choices"].empty()) {
                    std::string ai_content = response_json["choices"][0]["message"]["content"];
                    ...
                    std::cerr << "Unexpected response format: " << response_str << std::endl;
                }
